...
public class ABB<K, V> implements IMapeamento<K, V>{
    ...
    public Lista<V> recortar(K chaveDeOnde, K chaveAteOnde) {
    	...
    	Lista<V> resultado = new Lista<V>();
		recortarAux(this.raiz, chaveDeOnde, chaveAteOnde, resultado);

		return resultado;
	}

	protected void recortarAux(No<K, V> no, K chaveDeOnde, K chaveAteOnde, Lista<V> lista){
		if (no == null){
			return;
		}

		// Comparar usando o comparador, não diretamente no no.chave
		int comparacaoInicio = comparador.compare(chaveDeOnde, no.getChave());
		int comparacaoFim = comparador.compare(chaveAteOnde, no.getChave());
		
		// Se a chave atual é maior que o fim, vá para esquerda
		if (comparacaoFim < 0) {
			recortarAux(no.getEsquerda(), chaveDeOnde, chaveAteOnde, lista);
		}
		// Se a chave atual está no intervalo
		else if (comparacaoInicio <= 0 && comparacaoFim >= 0) {
			// Percorra esquerda
			recortarAux(no.getEsquerda(), chaveDeOnde, chaveAteOnde, lista);
			// Adicione o item atual
			lista.inserir(no.getItem());
			// Percorra direita
			recortarAux(no.getDireita(), chaveDeOnde, chaveAteOnde, lista);
		}
		// Se a chave atual é menor que o início, vá para direita
		else if (comparacaoInicio > 0) {
			recortarAux(no.getDireita(), chaveDeOnde, chaveAteOnde, lista);
		}
	}
